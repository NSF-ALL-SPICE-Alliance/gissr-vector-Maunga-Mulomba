...
```{r}
my_inat_obs_df <- get_inat_obs(taxon_id = 5438, 
                            place_id = 48731,
                            quality = "research",
                            geo = TRUE, 
                            maxresults = 100) 

save(inat_obs_df, file = "livingstone_hornbills_inat.Rdata")
```


```{r}
my_inat_obs_sf <-  my_inat_obs_df %>% 
  select(longitude, latitude, datetime, common_name, scientific_name, image_url, user_login) %>% 
  st_as_sf(coords=c("longitude", "latitude"), crs=4326)
...
```{r}
# retrieving data of streets on livingstone
...
# retrieving data of national park on livingstone
livingstone_np <- livingstone_bb %>%
  opq() %>%
  add_osm_feature(key = "boundary", value = "national_park") %>%
  osmdata_sf()

# retrieving data of protected areas on livingstone
livingstone_protected <- livingstone_bb %>%
  opq() %>%
  add_osm_feature(key = "boundary", value = "protected_area") %>%
  osmdata_sf()
...
```{r}
# visualising all retrieved features over each other to form a map of Livingstone
p <- ggplot() +
  geom_sf(data = livingstone_streets$osm_lines, inherit.aes = FALSE, color = "#ffbe7f", size = .4, alpha = .8) +
  geom_sf(data = livingstone_small_streets$osm_lines, inherit.aes = FALSE, color = "#a6a6a6", size = .2, alpha = .8) +
  geom_sf(data = livingstone_np$osm_polygons, inherit.aes = FALSE, color = "brown", size = .2, alpha = .8) +
  geom_sf(data = livingstone_protected$osm_polygons, inherit.aes = FALSE, color = "green", size = .2, alpha = .8) +
  geom_point(data = my_inat_obs_df, aes(x = longitude, y = latitude, color = common_name, text = common_name)) + 
  geom_sf_text(size = 1, data = maui_protected$osm_polygons, aes(label = name)) + 
  coord_sf(xlim = c(25.70143, 26.02143), ylim = c(-18.01313, -17.69314), expand = TRUE) + 
  ggtitle("Hornibills in Livingstone", subtitle = "Based on iNaturalist Data as of October 2024") +
  theme_bw() +
  labs(
    color="Common Name", #this changes the title of our legend
    x = "Longitude",
...
```{r}
my_inat_obs_popup_sf <- my_inat_obs_sf %>% 
  mutate(popup_html = paste0("<p><b>", common_name, "</b><br/>",
                             "<i>", scientific_name, "</i></p>",
                             "<p>Observed: ", datetime, "<br/>",
                             "User: ", user_login, "</p>",
...
```{r}
htmltools::p("iNaturalist Observations of Hornibills in livingstone",
             htmltools::br(),
             my_inat_obs_popup_sf$datetime %>% 
               as.Date() %>% 
               range(na.rm = TRUE) %>% 
               paste(collapse = " to "),
             style = "font-weight:bold; font-size:110%;")
...
```{r}
leaflet(my_inat_obs_sf) %>% 
  setView(lng = 25.83, lat = -17.88, zoom = 13.5)%>%
  addTiles() %>% 
  addCircleMarkers(data = my_inat_obs_popup_sf,
                   popup = ~popup_html, 
                   radius = 5)
```
